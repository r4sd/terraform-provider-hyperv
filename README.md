...
> ⚠️ **同一 VM 内で NIC 名を重複させた構成は CIM 経路では非対応。** CIM には NIC の
> 作成順を表す決定的なキーが無く、read 結果を config の並びへ寄せる際に名前で突き合わせる
> ため、同名 NIC は位置が入れ替わりうる。switch や MAC が異なる同名 NIC を作ると
> 恒常 diff になる。NIC 名は VM 内で一意にすること。
